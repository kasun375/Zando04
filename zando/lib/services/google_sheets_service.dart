...
class GoogleSheetsService {
  final String defaultProductSheetId =
      '1yk_RTBEhvyr-2AUqBCCrMOhuz_-yuOMl9xYEcnKgOUY';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return rows.where((row) => row.length >= 6).map((row) {
          ...
          List<String> imageUrls = rawImageUrl
              .split(',')
              .map((url) => url.trim())
          ...
          if (row.length > 7) {
            ...
            if (rawGallery.isNotEmpty) {
              final parsedGallery = rawGallery
                  .split(',')
  ...
  Future<List<BannerModel>> fetchBanners({String? sheetId, String? gid}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        String csvData = response.body;
        debugPrint(
          'DEBUG: Banner CSV Data (first 100 chars): ${csvData.substring(0, csvData.length > 100 ? 100 : csvData.length)}',
        );
        ...
        for (var row in rows) {
          try {
            ...
            String title = row.length > 1
                ? (row[1]?.toString() ?? '').trim()
                : '';
            String convertedUrl = ImageUtils.convertToDirectLink(rawImageUrl);

            debugPrint(
              'DEBUG: Banner processing - Title: $title, Raw URL: $rawImageUrl',
            );
        ...
        throw Exception(
          'Failed to load Banner Google Sheet: ${response.statusCode}',
        );
      }
    } catch (e) {
